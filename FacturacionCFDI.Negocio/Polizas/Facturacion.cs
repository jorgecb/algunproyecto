...
namespace FacturacionCFDI.Negocio.Polizas
{
    public class Facturacion
    {
        ...
        /// Constructor
        ///</summary>
        public Facturacion(BaseDatos baseDatos)
        {
        ...
        public async Task<GenericResponse> ProcesoFacturacion()
        {
            try
            {
                int count = 0;
                ...
                do {
                    ...
                    List<LogTimbrado> rows = _baseDatos.Select<LogTimbrado>("SELECT * FROM FACTURACION_LOGTIMBRADO WHERE ERROR LIKE ('%CFDI33132%') AND ESTATUS = 0");
                    if (rows.Count != 0)
                    {
                        Console.WriteLine("Regresar ciclo para facturas los RFC erroneos con el RFC GENERICO");
                ...
                {
                    Codigo = 1,
                    Mensaje = $"Todo ha salido bien, se han refacturado {count2}/{count} facturas"
                };
            }
            catch (Exception ex)
            {
